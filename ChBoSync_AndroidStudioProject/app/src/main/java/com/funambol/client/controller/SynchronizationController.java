...
public class SynchronizationController extends BasicSynchronizationController
     ...
     *
     */
    public synchronized void synchronize(String syncType, List<AppSyncSource> syncSources, int delay) {
    ...
    protected synchronized void forceSynchronization(String syncType, List<AppSyncSource> syncSources,
                                                     boolean refresh, int direction,
                                                     int delay, boolean fromOutside)
    {
        // Search if at least one of the selected sources has a warning on the
        // first sync
    	List<AppSyncSource> sourcesWithQuestion = new Vector<AppSyncSource>();
        ...
        if (screen == null && sourcesWithQuestion.size() > 0) {
            ...
            sourcesWithQuestion.toArray(dialogDependentSources);
            ...
            */
            ...
            if (sourcesWithQuestion.isEmpty()) {
                ...
                continueSynchronizationAfterBandwithSaverDialog(syncType, syncSources, refresh, direction, delay, fromOutside, false);
            } else {
                if (Log.isLoggable(Log.DEBUG)) {
    ...
    public synchronized void continueSynchronizationAfterBandwithSaverDialog(String syncType,
                                                    List<AppSyncSource> syncSources,
                                                    boolean refresh,
                                                    int direction,
                                                    int delay,
                                                    boolean fromOutside,
    ...
    {
    	List<AppSyncSource> filteredSources = new Vector<AppSyncSource>();
    	List<AppSyncSource> sourcesWithQuestion = new Vector<AppSyncSource>();
        ...
        for(AppSyncSource appSource : syncSources) {
            ...
            if (synced || warning == null) {
                ...
                sourcesWithQuestion.add(appSource);
            }
        }
                                                ...
                                                boolean continueSyncFromDialog)
    {
        ...
        Iterator<AppSyncSource> sources = filteredSources.iterator();
        while(sources.hasNext()) {
            AppSyncSource appSource = (AppSyncSource) sources.next();
    ...
    private class FirstSyncRequest {
        ...
        public List<AppSyncSource> filteredSources;
