...
public abstract class BasicSynchronizationController
    ...
    private List<AppSyncSource> localStorageFullSources = new Vector<AppSyncSource>();
    private List<AppSyncSource> serverQuotaFullSources = new Vector<AppSyncSource>();
    ...
    protected void checkSourcesForStorageOrQuotaFullErrors(List<AppSyncSource> sources) {
        for (AppSyncSource appSource : sources) {
     ...
     */
    protected void displayStorageLimitWarning(List<AppSyncSource> localStorageFullSources) {
     ...
     */
    protected void displayServerQuotaWarning(List<AppSyncSource> serverQuotaFullSources) {
     ...
     *         passed the check
     */
    protected List<AppSyncSource> applyBandwidthSaver(List<AppSyncSource> syncSources, List<AppSyncSource> sourcesWithQuestion, String syncType) {
        ...
        if (configuration.getBandwidthSaverActivated() && !networkStatus.isWiFiConnected()) {
            ...
            if (!MANUAL.equals(syncType)) {
            	List<AppSyncSource> prefilteredSources = new Vector<AppSyncSource>();
                for (AppSyncSource appSource : syncSources) {
                    ...
                    if (appSource.getBandwidthSaverUse()) {
                        ...
                        // It's OK
                        prefilteredSources.add(appSource);
                    }
                }
