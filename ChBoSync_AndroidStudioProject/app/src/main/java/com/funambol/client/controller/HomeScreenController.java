...
public class HomeScreenController extends SynchronizationController {
    ...
    protected List<AppSyncSource>             items = null;
    ...
    public void syncEnded() {
        ...
        for(int i=0;i<items.size();++i) {
            AppSyncSource appSource = (AppSyncSource)items.get(i);
    ...
    public List<AppSyncSource> getVisibleItems() {
        return items;
    }
    ...
    public void buttonSelected(int index) {
        ...
        AppSyncSource source = (AppSyncSource) items.get(index);
    ...
    protected void lockHomeScreen(Vector sources) {
        ...
        for(int j=0;j<items.size();++j) {
            AppSyncSource appSource = (AppSyncSource) items.get(j);
    ...
    public void updateEnabledSources() {
        ...
        Iterator<AppSyncSource> sources = this.items.iterator();
        boolean atLeastOneEnabled = false;
        while (sources.hasNext()) {
            AppSyncSource appSource = sources.next();
    ...
    public void syncMenuSelected() {
        if (selectedIndex != -1) {
            AppSyncSource appSource = (AppSyncSource)items.get(selectedIndex);
    ...
    public void aloneSourcePressed() {
        ...
        if (isSynchronizing()) {
            ...
            AppSyncSource appSource = (AppSyncSource)items.get(0);
            syncSource(MANUAL, appSource);
        }
    }
    ...
    public void syncAllSources(String syncType) {
        ...
        List<AppSyncSource> sources = new ArrayList<AppSyncSource>();        
        for(int i=0;i<items.size();++i) {
            AppSyncSource appSource = items.get(i);
            if (appSource.getConfig().getEnabled() && appSource.isWorking()) {
    ...
    public synchronized void synchronize(String syncType, List<AppSyncSource> syncSources) {
        ...
        if (MANUAL.equals(syncType)) {
            ...
            for(AppSyncSource appSource : syncSources) {
    ...
    public void gotoMenuSelected() {
        if (selectedIndex != -1) {
            AppSyncSource source = this.items.get(this.selectedIndex);
    ...
    protected void unlockHomeScreen() {
        ...
        for(AppSyncSource appSource : this.items) {
            // If this source is in sources then we shall enable it,
            // otherwise we must disable it
    ...
    private int getFirstActiveItemIndex() {
        ...
        for (int i=0; i<size; i++) {
            AppSyncSource source = this.items.get(i);
    ...
    private void computeVisibleItems() {
        ...
        items = new Vector<AppSyncSource>();
        ...
        while (sources.hasMoreElements()) {
            ...
            if (controller.isVisible(appSource)) {
                ...
                items.set(index, appSource);
            }
        }
    }

    /**
     * Set length of AppSources-List to realSize
     * Actually a re-implementation of an original Vector.setSize() call
     * @param realSize
     */
	private void correctSourcesList(int realSize) {
		if (realSize<this.items.size()) {
        	this.items=this.items.subList(0, realSize);
        } else {
        	while (this.items.size()<realSize) this.items.add(null);
        }
	}
    ...
    private void setSelected(int index, boolean fromUi) {
        ...
        if ((selectedIndex != index) &&
            ...
            AppSyncSource oldAppSource = this.items.get(this.selectedIndex);
        ...
        AppSyncSource appSource = this.items.get(index);
        if (!appSource.isEnabled() || !appSource.isWorking()) {
